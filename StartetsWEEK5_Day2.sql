...
SELECT * FROM table_constraints where TABLE NAME='users';
...
-- 오류 ISERT INTO BOARD(TITLE,WRITER) VALUES ('새제목','MARIA2');
...
-- first_name이 kelly 사원과 동일 부서근무자;
-- 단일행 subquery --> '='
SELECT department_id,salary,first_name 
FROM employees WHERE department_id=(SELECT department_id FROM employees WHERE first_name="kelly");

-- 다중행 subquery
SELECT department_id,salary,first_name 
FROM employees WHERE department_id IN (SELECT department_id FROM employees WHERE first_name="peter");
...
-- 이름 최소급여
SELECT first_name,MAX(salary) FROM employees WHERE salary=(SELECT Min(salary) FROM employees);

-- peter 같은 부서이고 같은 입사일에 입사한 부서코드 급여 이름 조회
...
FROM employees WHERE (department_id,job_id)
